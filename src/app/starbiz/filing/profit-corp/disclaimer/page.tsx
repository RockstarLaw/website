/**
 * Profit Corp Disclaimer Page — R8-1
 *
 * 1:1 clone of the real Sunbiz profit_file.html disclaimer page.
 * The HTML is stored as a transformed static asset at
 * public/sunbiz/profit-corp-disclaimer.html and served verbatim via
 * dangerouslySetInnerHTML — no React component wrappers, no StarBizShell,
 * no EFilingShell.  The cloned HTML carries its own complete chrome
 * (header, breadcrumbs, disclaimer box, two-column form, footer, dosbanner).
 *
 * Auth gate is enforced here before any HTML is returned.
 *
 * Transformations already baked into profit-corp-disclaimer.html (SESSION_HANDOFF §2):
 ...
 *   • Left form action → /starbiz/filing/profit-corp/form (method=POST, filing_type=DOMP).
 *   • Right form action → /starbiz/filing/profit-corp/form (method=POST, filing_type=NONE).
 *   • Both forms active (unlike LLC chain where Update Filing was disabled).
...
export default async function ProfitCorpDisclaimerPage() {
  // ── Auth check ──────────────────────────────────────────────────────────────
  ...
  const fullHtml = readFileSync(
    join(process.cwd(), "public/sunbiz/profit-corp-disclaimer.html"),
    "utf-8",
  );
