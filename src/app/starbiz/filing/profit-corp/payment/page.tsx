/**
 * Profit Corp Payment Page — Retrofit R8-5
 *
 * 1:1 clone of the real Sunbiz corenrtn.exe payment page for the Profit
 * Corporation formation chain.
 * HTML template stored at public/sunbiz/profit-corp-payment.html.
 ...
 * Profit Corp charge formula (same as R8-4 filing-info):
 *   base = 70.00
 *   + 8.75 if form_data.cos_num_flag  === 'Y'
 *   + 8.75 if form_data.cert_num_flag === 'Y'
 *
 ...
 *
 * Scoped <style> block: verbatim copy of LLC payment/page.tsx (R7b-5).
 *
 * Mirrors LLC payment/page.tsx (R7b-5) with:
 *   • filing_type = 'corp' in session query
 *   • redirect fallback = /starbiz/filing/profit-corp/disclaimer
 *   • template = public/sunbiz/profit-corp-payment.html
 *   • charge formula = Profit Corp ($70 base)
 */
...
export default async function ProfitCorpPaymentPage() {
  // ── Auth check ──────────────────────────────────────────────────────────────
  ...
  // ── Compute Profit Corp filing charge (same formula as R8-4 filing-info) ─────
  let charge = 70.00;
  ...
  let html = readFileSync(
    join(process.cwd(), "public/sunbiz/profit-corp-payment.html"),
    "utf-8",
  );
  ...
  return (
    <>
      ...
      {/*
       * Scoped reset — verbatim copy of LLC payment/page.tsx (R7b-5).
       * Same CSS leakage fixes + .efiledata alias.
       */}
      <style>{`
        body {
