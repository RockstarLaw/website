...
import { submitLLCFormation } from "@/app/starbiz/filing/llc/actions";
...
function hasLLCSuffix(n: string): boolean { return /\b(llc|l\.l\.c\.|limited liability company)\b/i.test(n); }
function validZip(z: string): boolean { return /^\d{5}(-\d{4})?$/.test(z.trim()); }
...
function validateStep(step: number, d: WizardData, persons: PersonRow[]): Errors {
  ...
  if (step === 4) {
    ...
    persons.forEach((p, i) => {
      if (p.name.trim() && !p.street.trim()) e[`person_${i}_street`] = "Address required.";
      if (p.name.trim() && !p.city.trim())   e[`person_${i}_city`]   = "City required.";
      if (p.name.trim() && !p.state.trim())  e[`person_${i}_state`]  = "State required.";
      if (p.name.trim() && !p.zip.trim())    e[`person_${i}_zip`]    = "ZIP required.";
    });
...
function Field({ label, help, helpLabel, children, error }: { label: string; help?: string; helpLabel?: string; children: React.ReactNode; error?: string }) {
  return (
    <tr>
...
function Step1({ d, up, e }: { d: WizardData; up: <K extends keyof WizardData>(k: K, v: WizardData[K]) => void; e: Errors }) {
  return (
    <div>
      <div style={sSection}>Article I — LLC Identity</div>

      {/* Names-must-be-unique notice */}
      ...
      <table cellPadding={0} cellSpacing={0} style={{ width: "100%" }}>
        <tbody>
          <Field label="LLC Name *"
            help="Your LLC must include 'LLC,' 'L.L.C.,' or 'Limited Liability Company' in its name. Names must be unique across all filed entities — case and spacing differences are ignored, so 'Acme LLC' and 'ACME LLC' count as the same name. If your chosen name is already taken, you'll be prompted to pick another at submit."
            helpLabel="LLC Name"
            error={e.name}>
            <input type="text" value={d.name} onChange={ev => up("name", ev.target.value)}
              style={{ ...sInput, width: "320px" }} maxLength={120}
              placeholder="e.g. Sunshine Legal Services, LLC" autoFocus />
          ...
          <Field label="Management Structure *"
            ...
            <label style={sLabel}>
              <input type="radio" name="mgmt" value="member-managed" checked={d.managementStructure === "member-managed"} onChange={() => up("managementStructure", "member-managed")} />
              {" "}Member-Managed
            </label>
            &nbsp;&nbsp;&nbsp;
            <label style={sLabel}>
              <input type="radio" name="mgmt" value="manager-managed" checked={d.managementStructure === "manager-managed"} onChange={() => up("managementStructure", "manager-managed")} />
              {" "}Manager-Managed
            </label>
          </Field>
          ...
          <Field label="Purpose"
            ...
            error={e.purpose}>
            <input type="text" value={d.purpose} onChange={ev => up("purpose", ev.target.value)} style={{ ...sInput, width: "320px" }} maxLength={500} placeholder="any lawful purpose" />
          </Field>
          ...
          <Field label="Effective Date"
            ...
            <input type="date" value={d.effectiveDate} onChange={ev => up("effectiveDate", ev.target.value)} style={sInput} />
            <span style={{ fontFamily: "Arial", fontSize: "10px", color: "#666", marginLeft: "6px" }}>(leave blank = effective immediately)</span>
          </Field>
        </tbody>
      </table>
    </div>
...
function Step2({ d, up, e }: { d: WizardData; up: <K extends keyof WizardData>(k: K, v: WizardData[K]) => void; e: Errors }) {
  return (
    <div>
      <div style={sSection}>Article II — Principal Office</div>
      <table cellPadding={0} cellSpacing={0} style={{ width: "100%" }}>
        <tbody>
          <Field label="Street Address *" help="The primary business address. May be outside Florida. P.O. Boxes not accepted." error={e.principalStreet}>
            <input type="text" value={d.principalStreet} onChange={ev => up("principalStreet", ev.target.value)} style={{ ...sInput, width: "300px" }} />
          </Field>
          <Field label="City *" error={e.principalCity}><input type="text" value={d.principalCity} onChange={ev => up("principalCity", ev.target.value)} style={{ ...sInput, width: "180px" }} /></Field>
          <Field label="State *" error={e.principalState}><input type="text" value={d.principalState} onChange={ev => up("principalState", ev.target.value.toUpperCase())} style={{ ...sInput, width: "40px" }} maxLength={2} placeholder="FL" /></Field>
          <Field label="ZIP *" error={e.principalZip}><input type="text" value={d.principalZip} onChange={ev => up("principalZip", ev.target.value)} style={{ ...sInput, width: "90px" }} placeholder="32801" /></Field>
        </tbody>
      </table>
      ...
      <table cellPadding={0} cellSpacing={0} style={{ width: "100%" }}>
        <tbody>
          ...
          {!d.mailingIsSame && (
            <>
              <Field label="Mailing Street *" error={e.mailingStreet}><input type="text" value={d.mailingStreet} onChange={ev => up("mailingStreet", ev.target.value)} style={{ ...sInput, width: "300px" }} /></Field>
              <Field label="City *" error={e.mailingCity}><input type="text" value={d.mailingCity} onChange={ev => up("mailingCity", ev.target.value)} style={{ ...sInput, width: "180px" }} /></Field>
              <Field label="State *" error={e.mailingState}><input type="text" value={d.mailingState} onChange={ev => up("mailingState", ev.target.value.toUpperCase())} style={{ ...sInput, width: "40px" }} maxLength={2} /></Field>
              <Field label="ZIP *" error={e.mailingZip}><input type="text" value={d.mailingZip} onChange={ev => up("mailingZip", ev.target.value)} style={{ ...sInput, width: "90px" }} /></Field>
            </>
          )}
        </tbody>
      </table>
...
function Step3({ d, up, e }: { d: WizardData; up: <K extends keyof WizardData>(k: K, v: WizardData[K]) => void; e: Errors }) {
  return (
    <div>
      <div style={sSection}>Article III — Registered Agent</div>
      <table cellPadding={0} cellSpacing={0} style={{ width: "100%" }}>
        <tbody>
          <Field label="RA Name *" help="The registered agent receives legal documents and official state correspondence. Must be a Florida resident or a Florida-registered entity." error={e.raName}>
            <input type="text" value={d.raName} onChange={ev => up("raName", ev.target.value)} style={{ ...sInput, width: "280px" }} autoFocus />
          </Field>
          <Field label="RA Street Address *" help="Must be a physical Florida address. P.O. Boxes not accepted. This address is publicly searchable." error={e.raStreet}>
            <input type="text" value={d.raStreet} onChange={ev => up("raStreet", ev.target.value)} style={{ ...sInput, width: "300px" }} />
          </Field>
          <Field label="City *" error={e.raCity}><input type="text" value={d.raCity} onChange={ev => up("raCity", ev.target.value)} style={{ ...sInput, width: "180px" }} /></Field>
          ...
          <Field label="ZIP *" error={e.raZip}><input type="text" value={d.raZip} onChange={ev => up("raZip", ev.target.value)} style={{ ...sInput, width: "90px" }} /></Field>
          <Field label="RA Email"><input type="email" value={d.raEmail} onChange={ev => up("raEmail", ev.target.value)} style={{ ...sInput, width: "220px" }} placeholder="optional" /></Field>
          <tr>
            <td colSpan={2} style={{ ...sTd, paddingTop: "10px" }}>
              <label style={{ ...sLabel, fontWeight: "normal", display: "flex", alignItems: "flex-start", gap: "6px" }}>
                <input type="checkbox" checked={d.raAccepted} onChange={ev => up("raAccepted", ev.target.checked)} style={{ marginTop: "2px" }} />
                <span>
...
function Step4({
  ...
  const pctSum = persons.reduce((acc, p) => { const v = parseFloat(p.ownershipPct); return acc + (isNaN(v) ? 0 : v); }, 0);
    ...
    <div>
      ...
      {persons.map((p, i) => (
        <div key={i} style={{ border: "1px solid #999", padding: "8px", marginBottom: "6px", backgroundColor: i % 2 === 1 ? "#FFFF99" : WHITE }}>
          ...
          <table cellPadding={0} cellSpacing={0}><tbody>
            <tr>
              <td style={sTd}>
                <span style={sLabel}>Name *{i === 0 && <HelpTip helpText="List the full legal name. For member-managed LLCs, list every owner. For manager-managed, list only the managers." label={`${personLabel.replace(/s$/, "")} Name`} />}</span><br />
              ...
              {isMemberManaged && (
                <td style={sTd}>
                  <span style={sLabel}>Ownership %{i === 0 && <HelpTip helpText="Your ownership share. Percentages can add up to less than 100% (reserved interests are normal). A warning appears if the total exceeds 100%." label="Ownership Percentage" />}</span><br />
                  <input type="number" step="0.01" min="0" max="100" value={p.ownershipPct} onChange={ev => updatePerson(i, "ownershipPct", ev.target.value)} style={{ ...sInput, width: "70px" }} placeholder="0.00" />
                  {" "}%
                </td>
              )}
            </tr>
            <tr>
              <td style={sTd} colSpan={3}>
                ...
                <span style={{ marginLeft: "8px" }}>
                  <input type="text" value={p.street} onChange={ev => updatePerson(i, "street", ev.target.value)} style={{ ...sInput, width: "160px" }} placeholder="Street" />
                  {" "}<input type="text" value={p.city} onChange={ev => updatePerson(i, "city", ev.target.value)} style={{ ...sInput, width: "110px" }} placeholder="City" />
                  {" "}<input type="text" value={p.state} onChange={ev => updatePerson(i, "state", ev.target.value.toUpperCase())} style={{ ...sInput, width: "28px" }} maxLength={2} placeholder="ST" />
                  {" "}<input type="text" value={p.zip} onChange={ev => updatePerson(i, "zip", ev.target.value)} style={{ ...sInput, width: "70px" }} placeholder="ZIP" />
                </span>
      ...
      {isMemberManaged && pctSum > 100 && (
        <p style={{ fontFamily: "Arial", fontSize: "11px", color: "#cc6600", border: "1px solid #cc6600", padding: "3px 8px", backgroundColor: "#FFFAED" }}>
          ⚠ Total ownership exceeds 100% ({pctSum.toFixed(2)}%). This will be noted in the filing record.
        </p>
      )}
      ...
      <table cellPadding={0} cellSpacing={0} style={{ width: "100%" }}><tbody>
        <Field label="Organizer Name *" help="The person submitting this filing. Doesn't have to be a member or manager — they're the filer of record." error={e.organizerName}>
        ...
        <tr>
          ...
          <td style={sTd}>
            <input type="text" value={d.organizerStreet} onChange={ev => up("organizerStreet", ev.target.value)} style={{ ...sInput, width: "160px" }} placeholder="Street" />
            {" "}<input type="text" value={d.organizerCity} onChange={ev => up("organizerCity", ev.target.value)} style={{ ...sInput, width: "110px" }} placeholder="City" />
            {" "}<input type="text" value={d.organizerState} onChange={ev => up("organizerState", ev.target.value.toUpperCase())} style={{ ...sInput, width: "28px" }} maxLength={2} placeholder="ST" />
            {" "}<input type="text" value={d.organizerZip} onChange={ev => up("organizerZip", ev.target.value)} style={{ ...sInput, width: "70px" }} placeholder="ZIP" />
            <Err msg={e.organizerStreet || e.organizerCity || e.organizerState || e.organizerZip} />
          </td>
        </tr>
        <Field label="FEI / EIN" help="Optional at formation. Obtain your EIN from the IRS after the LLC is formed.">
      ...
      <table cellPadding={4} cellSpacing={0} style={{ width: "100%", border: "1px solid #999", backgroundColor: WHITE, fontFamily: "Arial", fontSize: "11px" }}><tbody>
...
export function LLCWizard() {
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      const result: LLCSubmitResult = await submitLLCFormation(data, persons);
      if (result.error) {
      ...
      // On success, server action calls redirect() — no client handling needed.
    });
  }
      ...
      <div style={{ display: "flex", gap: "8px", marginTop: "14px", paddingTop: "10px", borderTop: "1px solid #ccc", alignItems: "center" }}>
        ...
        {step === 4 && (
          <button type="button" onClick={handleSubmit} disabled={isPending}
            ...
            {isPending ? "Filing…" : "File Articles of Organization — $125.00"}
          </button>
        )}
        {isPending && (
